...
class CooperativeTrainer:
    ...
    def save_models(self, filepath_prefix):
        """Save both agent models and shared backbone (lightweight, for best model only)"""
    ...
    def save_complete_checkpoint(self, filepath_prefix, episode, training_state):
        """Save complete training state for resuming training"""
        os.makedirs(os.path.dirname(filepath_prefix) or '.', exist_ok=True)
        
        checkpoint = {
            # Model states
            'shared_backbone': self.fire_agent.policy_net.shared_backbone.state_dict(),
            'fire_partner_encoder': self.fire_agent.policy_net.partner_encoder.state_dict(),
            'fire_agent_head': self.fire_agent.policy_net.agent_head.state_dict(),
            'fire_actor': self.fire_agent.policy_net.actor.state_dict(),
            'fire_critic': self.fire_agent.policy_net.critic.state_dict(),
            'water_partner_encoder': self.water_agent.policy_net.partner_encoder.state_dict(),
            'water_agent_head': self.water_agent.policy_net.agent_head.state_dict(),
            'water_actor': self.water_agent.policy_net.actor.state_dict(),
            'water_critic': self.water_agent.policy_net.critic.state_dict(),
            
            # Optimizer states
            'shared_optimizer': self.shared_optimizer.state_dict(),
            'fire_optimizer': self.fire_optimizer.state_dict(),
            'water_optimizer': self.water_optimizer.state_dict(),
            
            # Training progress
            'episode': episode,
            'fire_epsilon': self.fire_agent.epsilon,
            'water_epsilon': self.water_agent.epsilon,
            
            # Metrics and trackers
            'episode_rewards': list(training_state.get('episode_rewards', [])),
            'success_tracker': list(training_state.get('success_tracker', [])),
            'fire_success_tracker': list(training_state.get('fire_success_tracker', [])),
            'water_success_tracker': list(training_state.get('water_success_tracker', [])),
            'cooperation_events_tracker': list(training_state.get('cooperation_events_tracker', [])),
            'best_avg_reward': training_state.get('best_avg_reward', -float('inf')),
            
            # Agent-specific metrics
            'fire_cooperation_events': self.fire_agent.cooperation_events,
            'fire_plate_activations': self.fire_agent.plate_activations,
            'fire_safety_violations': self.fire_agent.safety_violations,
            'fire_safe_actions_taken': self.fire_agent.safe_actions_taken,
            'water_cooperation_events': self.water_agent.cooperation_events,
            'water_plate_activations': self.water_agent.plate_activations,
            'water_safety_violations': self.water_agent.safety_violations,
            'water_safe_actions_taken': self.water_agent.safe_actions_taken,
        ...
        # Save checkpoint
        checkpoint_path = f"{filepath_prefix}_checkpoint_ep{episode}.pt"
        torch.save(checkpoint, checkpoint_path)
        
        # Also save latest checkpoint for easy resume
        latest_path = f"{filepath_prefix}_checkpoint_latest.pt"
        torch.save(checkpoint, latest_path)
        
        # Save replay buffers separately (can be large)
        buffer_path = f"{filepath_prefix}_buffers_ep{episode}.pkl"
        with open(buffer_path, 'wb') as f:
            pickle.dump({
                'fire_buffer': list(self.fire_agent.replay_buffer.buffer),
                'water_buffer': list(self.water_agent.replay_buffer.buffer)
            }, f)
        
        # Save latest buffers
        latest_buffer_path = f"{filepath_prefix}_buffers_latest.pkl"
        with open(latest_buffer_path, 'wb') as f:
            pickle.dump({
                'fire_buffer': list(self.fire_agent.replay_buffer.buffer),
                'water_buffer': list(self.water_agent.replay_buffer.buffer)
            }, f)
        
        print(f"Complete checkpoint saved: episode {episode}")
        return checkpoint_path
    
    def load_complete_checkpoint(self, filepath_prefix):
        ...
        try:
            ...
            if checkpoint.get('state_dim', 52) != 52:
                print(f"WARNING: Checkpoint has different state dimension. Starting fresh.")
                return None
            
            # Load model states
            self.fire_agent.policy_net.shared_backbone.load_state_dict(checkpoint['shared_backbone'])
            self.fire_agent.policy_net.partner_encoder.load_state_dict(checkpoint['fire_partner_encoder'])
            self.fire_agent.policy_net.agent_head.load_state_dict(checkpoint['fire_agent_head'])
            self.fire_agent.policy_net.actor.load_state_dict(checkpoint['fire_actor'])
            self.fire_agent.policy_net.critic.load_state_dict(checkpoint['fire_critic'])
            
            self.water_agent.policy_net.partner_encoder.load_state_dict(checkpoint['water_partner_encoder'])
            self.water_agent.policy_net.agent_head.load_state_dict(checkpoint['water_agent_head'])
            self.water_agent.policy_net.actor.load_state_dict(checkpoint['water_actor'])
            self.water_agent.policy_net.critic.load_state_dict(checkpoint['water_critic'])
            
            # Load optimizer states
            self.shared_optimizer.load_state_dict(checkpoint['shared_optimizer'])
            self.fire_optimizer.load_state_dict(checkpoint['fire_optimizer'])
            self.water_optimizer.load_state_dict(checkpoint['water_optimizer'])
            
            # Load training progress
            self.fire_agent.epsilon = checkpoint['fire_epsilon']
            self.water_agent.epsilon = checkpoint['water_epsilon']
            
            # Load agent metrics
            self.fire_agent.cooperation_events = checkpoint.get('fire_cooperation_events', 0)
            self.fire_agent.plate_activations = checkpoint.get('fire_plate_activations', 0)
            self.fire_agent.safety_violations = checkpoint.get('fire_safety_violations', 0)
            self.fire_agent.safe_actions_taken = checkpoint.get('fire_safe_actions_taken', 0)
            
            self.water_agent.cooperation_events = checkpoint.get('water_cooperation_events', 0)
            self.water_agent.plate_activations = checkpoint.get('water_plate_activations', 0)
            self.water_agent.safety_violations = checkpoint.get('water_safety_violations', 0)
            self.water_agent.safe_actions_taken = checkpoint.get('water_safe_actions_taken', 0)
            ...
            if os.path.exists(latest_buffer_path):
                ...
                with open(latest_buffer_path, 'rb') as f:
                    ...
                    self.fire_agent.replay_buffer.buffer = deque(buffers['fire_buffer'], 
                                                                  maxlen=self.fire_agent.replay_buffer.buffer.maxlen)
                    self.water_agent.replay_buffer.buffer = deque(buffers['water_buffer'],
                                                                   maxlen=self.water_agent.replay_buffer.buffer.maxlen)
                print(f"Loaded {len(self.fire_agent.replay_buffer)} fire experiences, "
                      f"{len(self.water_agent.replay_buffer)} water experiences")
            
            training_state = {
                'episode': checkpoint['episode'],
                'episode_rewards': deque(checkpoint.get('episode_rewards', []), maxlen=100),
                'success_tracker': deque(checkpoint.get('success_tracker', []), maxlen=100),
                'fire_success_tracker': deque(checkpoint.get('fire_success_tracker', []), maxlen=100),
                'water_success_tracker': deque(checkpoint.get('water_success_tracker', []), maxlen=100),
                'cooperation_events_tracker': deque(checkpoint.get('cooperation_events_tracker', []), maxlen=100),
            ...
            print(f"Resuming training with epsilon={self.fire_agent.epsilon:.3f}, "
                  f"best_avg_reward={training_state['best_avg_reward']:.2f}")
...
def train_cooperative_agents(num_episodes=5000, render=True, use_wandb=True, checkpoint_frequency=50):
    """Main cooperative training loop with synchronized learning and checkpoint system"""
    ...
    # Try to load complete checkpoint first (includes everything)
    print("\nChecking for existing checkpoint...")
    training_state = trainer.load_complete_checkpoint("models/cooperative_agent")
    
    if training_state:
        # Resume from checkpoint
        start_episode = training_state['episode'] + 1
        episode_rewards = training_state['episode_rewards']
        success_tracker = training_state['success_tracker']
        fire_success_tracker = training_state['fire_success_tracker']
        water_success_tracker = training_state['water_success_tracker']
        cooperation_events_tracker = training_state['cooperation_events_tracker']
        ...
        print("\nNo checkpoint found. Starting fresh training with 52-dimensional state architecture")
    
    # Initialize W&B after checkpoint loading (to avoid duplicate runs)
    if use_wandb:
        resume_id = None
        if training_state:
            # Try to resume W&B run if available
            resume_id = "allow"  # Allow resuming if same run ID exists
        
        wandb.init(
            project="firewater-cooperative-rl",
            config={
    ...
    # Create training history log file
    log_dir = "models/training_logs"
    os.makedirs(log_dir, exist_ok=True)
    history_file = f"{log_dir}/training_history.jsonl"
    ...
    for episode in range(start_episode, num_episodes):
        ...
        history_entry = {
            ...
            'buffer_size_fire': len(fire_agent.replay_buffer),
            'buffer_size_water': len(water_agent.replay_buffer)
        }
        ...
        if episode % checkpoint_frequency == 0 and episode > 0:
            current_training_state = {
                ...
                'fire_success_tracker': fire_success_tracker,
                'water_success_tracker': water_success_tracker,
                'cooperation_events_tracker': cooperation_events_tracker,
                'best_avg_reward': best_avg_reward
            }
            trainer.save_complete_checkpoint("models/cooperative_agent", episode, current_training_state)
            print(f"[Checkpoint saved at episode {episode}]")
        
        # Save best model (lightweight version)
        if len(episode_rewards) == 100 and avg_reward > best_avg_reward:
            best_avg_reward = avg_reward
            trainer.save_models("models/cooperative_agent")
            # Also save a checkpoint when we get a new best
            current_training_state = {
                'episode_rewards': episode_rewards,
                'success_tracker': success_tracker,
                'fire_success_tracker': fire_success_tracker,
            ...
            trainer.save_complete_checkpoint("models/cooperative_agent", episode, current_training_state)
            print(f"New best average reward of {avg_reward:.2f}. Models and checkpoint saved.")
    ...
    trainer.save_complete_checkpoint("models/cooperative_agent", num_episodes - 1, final_training_state)
    print(f"Final checkpoint saved at episode {num_episodes - 1}")
    print("="*60)
...
def view_training_history(history_file="models/training_logs/training_history.jsonl", last_n=50):
    """View the last N entries from training history"""
    if not os.path.exists(history_file):
        print(f"No training history found at {history_file}")
        return
    
    print(f"\n{'='*80}")
    print(f"TRAINING HISTORY (Last {last_n} episodes)")
    print(f"{'='*80}")
    ...
    for entry in entries[-last_n:]:
        print(f"Ep {entry['episode']:5d} | "
              f"Reward: {entry['total_reward']:7.1f} | "
              f"Avg100: {entry['avg_reward_100']:7.1f} | "
              f"Success: {entry['success_rate_100']:.1%} | "
              f"Coop: {entry['cooperation_events']:2d} | "
              f"ε: {entry['fire_epsilon']:.3f}")
    
    print(f"{'='*80}\n")
    print(f"Total episodes logged: {len(entries)}")
    if entries:
        print(f"Training started: {entries[0]['timestamp']}")
        print(f"Last update: {entries[-1]['timestamp']}")
...
if __name__ == "__main__":
    ...
    try:
        while True:
            if mode == "human":
                ...
                result = train_cooperative_agents(
                    num_episodes=5000, render=USE_RENDERING, use_wandb=True, checkpoint_frequency=50)
                if result == "human":
                    mode = "human"
                elif result == "quit":
                    break
        ...
        print("All progress has been saved in the last checkpoint")
        print("="*60)
        print("\nTo resume training, simply run this script again.")
        print("The training will automatically continue from the last checkpoint.")
    except Exception as e:
        ...
        if USE_RENDERING and pg.get_init():
            pg.quit()
        print("Session ended.")
        print("\nTo view training history, run:")
        print("  from level_001RLv4 import view_training_history")
        print("  view_training_history()")